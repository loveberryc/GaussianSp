...
from x2_gaussian.arguments import ParamGroup, ModelParams, PipelineParams, ModelHiddenParams
from x2_gaussian.utils.plot_utils import show_one_volume, show_two_volume
from x2_gaussian.gaussian import GaussianModel, query, initialize_gaussian
...
def main(
    args, init_args: InitParams, model_args: ModelParams, pipe_args: PipelineParams, hyper: ModelHiddenParams,
):
    # Read scene
    data_path = args.data
    ...
    if not save_path:
        if osp.exists(osp.join(data_path, "meta_data.json")):
            ...
            save_path = osp.join(
                ...
                "init_" + osp.basename(data_path).split(".")[0] + "_debug.npy",
            )
        else:
...
if __name__ == "__main__":
    ...
    main(args, init_parser.extract(args), lp.extract(args), pp.extract(args), hp.extract(args))
